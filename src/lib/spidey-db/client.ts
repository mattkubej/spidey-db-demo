...
export default class SpideyDbClient {
	...
	async send(request: string[]): Promise<string | string[]> {
		return new Promise((resolve, reject) => {
			...
			const respRequest = encoder.encodeArray(request.map(encoder.encodeBulkString));
	...
	async getNeighbors(vertexKey: string, distance: number): Promise<string[]> {
		const response = await this.send(['GETNEIGHBORS', vertexKey, String(distance)]);
	...
	connect(): void {
		const client = createConnection(this.port, this.host, this.handleServerConnection.bind(this));
